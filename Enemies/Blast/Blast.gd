...
func _physics_process(delta):
	# Desce o Blast em 0.1px a cada frame.
	$".".position.y += 0.1
	
	# Move o Blast no eixo X de acordo com o valor da flag.
	if flag:
		$".".position.x -= 3
	else:
		$".".position.x += 3
		
	# Muda o sentido caso o Blast chegue nos limites da tela.
	...
	if time > clock:
		...
		var bullet = load("res://Enemies/Blast/Bullet/BlastBullet.tscn").instance()
		
		# Pega a posição do Blast.
		var position_x = $AnimatedSprite.get_parent().position[0]
		var position_y = $AnimatedSprite.get_parent().position[1]
		
		# Ajusta a posição da bala ao Blast.
...
# Deixa Blast invisível quando a animação de morte termina.
